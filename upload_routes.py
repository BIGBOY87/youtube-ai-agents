...
def _download_mp4(url):
    ...
    with requests.get(url,stream=True,timeout=120) as r:
        r.raise_for_status()
        if "text/html" in r.headers.get("content-type","").lower(): raise ValueError("URL returned HTML, not MP4.")
...
def register_upload_routes(app):
    ...
    def source_upload_private():
        ...
        if not source_url: return jsonify({"status":"blocked","reason":"Missing source_mp4_url."}),400
        path=None
        try:
