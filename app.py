...
app=Flask(__name__)
yt=YouTubeClient(); queue=ApprovalQueue()
...
def api_source_registry(): return jsonify(list_sources())

@app.route("/api/source/<video_id>")
def api_source(video_id):
    rec=find_by_video_id(video_id)
    if not rec: return jsonify({"error":"source record not found"}),404
    return jsonify(rec)
...
def api_source_analyze(video_id):
    ...
    if not rows: return jsonify({"error":"youtube video not found"}),404
...
def api_source_growth_loop():
    ...
    for rec in records:
        ...
        try:
            ...
            if not rows:
                results.append({"video_id":vid,"status":"not_found"}); continue
            analysis=analyze_video_performance(rows[0])
    ...
    queue.add(item); return jsonify(item)
